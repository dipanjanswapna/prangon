...
export async function getAboutPageContent(): Promise<AboutPageData> {
  ...
  if (docSnap.exists()) {
    ...
    // If document doesn't exist, you might want to return default data
    // and/or create the document with default data.
    console.log("No 'about' page document found! You may want to create it in the admin panel.");
    // Returning an empty/default structure to avoid breaking the page
    return {
...
export async function updateAboutPageContent(data: AboutPageData) {
  ...
  try {
    const reference = await docRef();
    await setDoc(reference, validation.data);
