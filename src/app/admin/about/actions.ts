...
export async function getAboutPageContent(): Promise<ActionResult<AboutPageData>> {
  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      const validation = aboutPageSchema.safeParse(docSnap.data());
      if (validation.success) {
        return { success: true, data: validation.data };
      } else {
        console.warn("Firestore 'about' data is invalid, returning default. Errors:", validation.error);
        // Fallback to default data if validation fails
      }
    } else {
  ...
  // Return default data if not found or invalid
...
export async function updateAboutPageContent(data: AboutPageData): Promise<ActionResult<{}>> {
  ...
  try {
    ...
    // This is now safe because we are not creating a client component error on the server.
    return { 
      success: false, 
      error: { 
        message: serverError.code === 'permission-denied' ? 'permission-denied' : serverError.message, 
        context: { 
          path: reference.path,
          operation: 'update',
